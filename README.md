...
⚠️ **IMPORTANT: Never push directly to `master` branch**

The `master` branch is automatically updated by Upptime workflows. All development work must be done in feature branches.

#### How Upptime Workflows Modify the Repository

Upptime's automated workflows commit directly to the `master` branch:

**Files Modified by Upptime:**

- `README.md` - Auto-updated with status badges and uptime stats (via `summary.yml` workflow)
- `history/*.yml` - Monitoring check results (via `uptime.yml` workflow)
- `graphs/*.png` - Response time graphs (via `graphs.yml` workflow)
- `api/*.json` - API endpoint data (via workflows)
- `gh-pages` branch - Status website deployment (via `site.yml` workflow, force-pushed)

**Workflow Commits:**

- Summary workflow: `:pencil: Update summary in README [skip ci] [upptime]`
- Uptime workflow: `:arrow_up: Update graph in README [skip ci] [upptime]`
- Response time workflow: `:bento: Update graphs [skip ci] [upptime]`

These commits happen automatically throughout the day as monitoring runs.
...
# 2. Make your changes (e.g., edit .upptimerc.yml)
vim .upptimerc.yml

# 3. Commit your changes
git add .
git commit -m "feat: add monitoring for new service"

# 4. Push to feature branch
git push origin feature/your-change-name

# 5. Create pull request to master
gh pr create --base master --head feature/your-change-name

# 6. Merge PR (avoid force-push to master)
gh pr merge --merge
```

**For Script or Workflow Changes:**

Same process as above. Always use feature branches.

**For Emergency Hotfixes:**

If you must push directly to master in an emergency:

```bash
# Pull latest Upptime auto-commits first
git pull --rebase origin master

# Make your urgent fix
vim .upptimerc.yml
git add .
git commit -m "hotfix: disable failing monitoring target"

# Push (will likely need rebase if Upptime committed in the meantime)
...
**Problem:** Upptime workflows continuously commit to `master`, causing conflicts:

```
! [rejected]        master -> master (fetch first)
```

**Solution:** Use feature branches to avoid conflicts with automated commits.

**Never use `--force` on master** unless you understand the implications (you'll overwrite Upptime's automated status updates).

### Files Safe to Modify

**Configuration (requires feature branch):**

- `.upptimerc.yml` - Monitoring configuration
- `.github/workflows/*.yml` - Workflow definitions
- `scripts/*.sh` - Setup/maintenance scripts
- `.gitignore`, `.gitattributes` - Git configuration

**Do Not Modify (auto-generated by Upptime):**

- `README.md` - Status section (lines with `[skip ci] [upptime]` commits)
- `history/*.yml` - Monitoring data
- `api/*.json` - API endpoints
- `graphs/*.png` - Response time graphs
- Anything in `gh-pages` branch
